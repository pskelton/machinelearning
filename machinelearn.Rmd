...
The final model had an out of sample error of 0.8% on the testing data split.
...
modelfit
```
The model is reporting an accuracy of 99.3% Lets now compare our model performance against the data we split up for testing.
```{r}
...
confmat2$overall
```
We can see that the model has performed very well on the testing set. The out of sample error rate is about 0.8%
